...
面向住院病区的电子交班 MVP。系统先从医院现有软件导入患者、完整病历、今日医嘱和检验检查状态，再整理成结构化交班草稿，由交班医生核对确认，最后由接班医生点击接收。

当前版本提供模拟医院系统和本地文件两个导入入口，但只载入虚构演示快照，不读取真实患者文件，也暂不包含登录、正式 HIS 接口和线上部署。

## 已实现

- 5A 病区 10 位虚构患者的交班看板、重点筛选和搜索
- “先导入、后生成”的班次资料流程与导入批次记录
- HIS/EMR、今日医嘱、LIS/RIS 模拟来源和 JSON/CSV/XLSX 文件选择入口
- 字段覆盖预览；源资料没有的内容保持空白并提示白班医生确认
...
1. 以“交班医生·沈医生”身份打开“导入班次资料”。
2. 查看来源和字段覆盖预览，使用模拟医院系统或选择一个本地演示文件。
3. 导入 10 位虚构患者后点击“生成本班交班”。
4. 打开任意患者卡，查看原文、空字段提示、编辑草稿或补充事项。
5. 点击“核对无误，确认交班”。
6. 切换到“接班医生·何医生”，点击“接收本次交班”。
7. 接班后可继续处理跨班未完成事项并查看往期时间线。
...
src/app/api/shifts/.../imports/  模拟医院数据导入接口
